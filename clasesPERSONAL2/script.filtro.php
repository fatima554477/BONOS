        ...
        beforeSend: function(objeto){
            $("#loader2").html(
                '<div class="msg-actualizando2">' +
                    '<span class="loader2"></span> ⏳ ACTUALIZANDO...' +
                '</div>'
            ).fadeIn();
...
function restoreRowSelections2(){
    // Restaurar checkboxes de selección de fila
    ...
    // Restaurar STATUS_BONORECHAZO: confiar en el valor que viene del servidor
    $("[id^='STATUS_BONORECHAZO']").each(function(){
        var id = this.id.replace('STATUS_BONORECHAZO', '');
        var estaChecked = this.checked; // valor que renderizó el PHP desde la BD

        // Limpiar localStorage (el servidor ya tiene la verdad)
        localStorage.removeItem("rechazo_" + id);

        // Sincronizar vista visual con lo que vino del servidor
        if (estaChecked) {
            actualizarVistaRechazoBonoFiltro2(id, true);
        }
...
function STATUS_BONORECHAZO_filtro(STATUS_BONORECHAZO_id){
    ...
    // Aplicar vista inmediatamente sin esperar el AJAX
    ...
    $.ajax({
        ...
        error: function(){
            // Si falla el AJAX, revertir visualmente
            checkBox.checked = !checkBox.checked;
            actualizarVistaRechazoBonoFiltro2(STATUS_BONORECHAZO_id, checkBox.checked);
            mostrarActualizado2('❌ ERROR AL GUARDAR');
        }
    });
}
...
function asegurarModalRechazoPersonal(){
    ...
    $('body').append(
        '<div id="modalRechazoPersonal" class="modal" tabindex="-1" role="dialog" style="display:none;background:rgba(0,0,0,0.45);">' +
            '<div class="modal-dialog" role="document">' +
                '<div class="modal-content">' +
                    '<div class="modal-header">' +
                        '<h5 class="modal-title" id="modalRechazoPersonalLabel">Motivo del rechazo</h5>' +
                        '<button type="button" class="close" onclick="cerrarModalRechazoPersonal()" style="border:none;background:transparent;font-size:25px;line-height:1;">&times;</button>' +
                    '</div>' +
                    '<div class="modal-body">' +
                        '<input type="hidden" id="modal_rechazo_personal_id" />' +
                        '<input type="hidden" id="modal_rechazo_personal_tipo" />' +
                        '<textarea id="modal_rechazo_personal_texto" class="form-control" rows="4"></textarea>' +
                        '<div id="modal_rechazo_personal_mensaje" style="margin-top:8px;color:#666;"></div>' +
                    '</div>' +
                    '<div class="modal-footer">' +
                        '<button type="button" id="btn_guardar_rechazo_personal_modal" class="btn btn-primary">Guardar</button>' +
                        '<button type="button" class="btn btn-secondary" onclick="cerrarModalRechazoPersonal()">Cerrar</button>' +
                    '</div>' +
                '</div>' +
            '</div>' +
        '</div>'
...
function guardarMotivoRechazoPersonalModal(){
    ...
    $.ajax({
        ...
        method:'POST',
        data:{
            RECHAZO_MOTIVO_PERSONAL_id:idPersonal,
            RECHAZO_MOTIVO_PERSONAL_tipo:tipoPersonal,
            RECHAZO_MOTIVO_PERSONAL_text:motivo
        },
...
function verMotivoRechazoPersonal(idPersonal, tipoPersonal){
    ...
    $.ajax({
        ...
        method:'POST',
        data:{
            RECHAZO_MOTIVO_PERSONAL_VER_id:idPersonal,
            RECHAZO_MOTIVO_PERSONAL_VER_tipo:tipoPersonal
        },
        success:function(resp){
