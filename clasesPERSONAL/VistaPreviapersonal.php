...
<tr>
    ...
    <td width="70%" class="form-control">
        <select name="FORMA_PAGO" style="background:#daddf5">
            ...
            <option style="background:#f2b4f5" value="03 TRANSFERENCIA ELECTRÓNICA" '.($row["FORMA_PAGO"] == "03" ? "selected" : "").'>03 TRANSFERENCIA ELECTRÓNICA</option>
            <option style="background:#ddf5da" value="01 EFECTIVO" '.($row["FORMA_PAGO"] == "01" ? "selected" : "").'>01 EFECTIVO</option>
            <option style="background:#fceade" value="02 CHEQUE" '.($row["FORMA_PAGO"] == "02" ? "selected" : "").'>02 CHEQUE NOMINATIVO</option>
            <option style="background:#dee6fc" value="04 TARJETA DE CRÉDITO" '.($row["FORMA_PAGO"] == "04" ? "selected" : "").'>04 TARJETA DE CRÉDITO</option>
            <option style="background:#f6fcde" value="05 MONEDERO" '.($row["FORMA_PAGO"] == "05" ? "selected" : "").'>05 MONEDERO ELECTRÓNICO</option>
            <option style="background:#dee2fc" value="06 DINERO ELECTRÓNICO" '.($row["FORMA_PAGO"] == "06" ? "selected" : "").'>06 DINERO ELECTRÓNICO</option>
            <option style="background:#f9e5fa" value="08 VALES DE DESPENSA" '.($row["FORMA_PAGO"] == "08" ? "selected" : "").'>08 VALES DE DESPENSA</option>
            <option style="background:#eefcde" value="28 TARJETA DE DÉBITO" '.($row["FORMA_PAGO"] == "28" ? "selected" : "").'>28 TARJETA DE DÉBITO</option>
            <option style="background:#fcfbde" value="29 TARJETA DE SERVICIO" '.($row["FORMA_PAGO"] == "29" ? "selected" : "").'>29 TARJETA DE SERVICIO</option>
            <option style="background:#f9e5fa" value="99 OTRO" '.($row["FORMA_PAGO"] == "99" ? "selected" : "").'>99 OTRO</option>
        </select>
    </td>
</tr>
...
<div id="drop_file_zone" ondrop="upload_file(event, \'ADJUNTO_COMPROBANTEP\');" ondragover="return false" style="width:300px;"> <p>Suelta aquí o busca tu archivo</p> <p> <input class="form-control form-control-sm" id="ADJUNTO_COMPROBANTEP" type="text" onkeydown="return false" onclick="file_explorer(\'ADJUNTO_COMPROBANTEP\');" style="width:250px;" value="'.$valorADJUNTO_COMPROBANTEP.'" required /> </p> <input type="file" name="ADJUNTO_COMPROBANTEP" id="nono" multiple/> <div id="2ADJUNTO_COMPROBANTEP"> "'.$urlADJUNTO_COMPROBANTEP.'" </div> </div> 
	...
	';
	 ...
	 $output .= '<tr>  
			...
			<input type="hidden" value="'.$row["idevento"].'"  name="IPpersonal"  id="IPpersonal"/>
